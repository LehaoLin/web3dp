  ...
  <div class="home">
    <div class="content">
      ...
      <el-tooltip
        ...
        placement="bottom"
      >
        <el-button class="wallet-address" type="success" round>{{
          wallet.slice(0, 5) + "..." + wallet.slice(wallet.length - 5)
        }}</el-button>
      </el-tooltip>
      <el-button
        v-else
        ...
        >
          <template #default="scope">
            ...
            <el-button
              ...
              type="primary"
              size="small"
              @click="downloadClick(scope.$index)"
              >Download</el-button
            >
            <el-button
              link
              type="primary"
              size="small"
              @click="editLabelClick(scope.$index)"
              >Edit Label</el-button
            >
          </template>
        </el-table-column>
        <el-table-column
      ...
      <el-button
        v-if="!upload_mode && !searching_data"
        @click="get_more_data"
        round
        >Get More Data</el-button
      >
        ...
        <el-row>
          <el-col :span="16">
            ...
            <div>
              ...
              <div>
                <el-button @click="export_poster_to_webp"
                  >Screenshot for model as the poster</el-button
                >
              </div>
            </div>
          </el-col>
          <el-col :span="8">
            ...
            >
              <el-input
                v-model="nft_storage_api_key"
                placeholder="Please input your NFT.storage API key for store the model file"
                clearable
              />
            </el-tooltip>
            ...
            <el-input
              v-model="model_name"
              placeholder="Please input your model name"
              clearable
            />
            ...
            <span v-if="metadata"
              >The file is stored in {{ metadata.url }}</span
            >

            <br />
            <br />
            <center>
              <a href="#" class="btnbtn" @click="mint2nft">Mint</a>
            </center>
          </el-col>
        </el-row>
      </div>
    ...
    <div class="demo">
      <model-viewer
        ...
        touch-action="pan-y"
      >
        <div class="annotation" v-if="upload_model_file">
          This is the preview of your selected model
        </div>
        <div class="annotation" v-if="default_model">
          This is the default model for demo
        </div>
      </model-viewer>
    </div>
  </div>
...
export default {
  ...
  mounted() {
    ...
    this.contract = new this.maticWeb3.eth.Contract(
      myContractAbi,
      myContractAddress
    );
    ...
    this.label_contract = new this.maticWeb3.eth.Contract(
      label_contract_abi,
  ...
  methods: {
    ...
    async search() {
      ...
      if (this.search_select == "1") {
        ...
        if (!isNaN(this.search_input)) {
          ...
          if (
            parseInt(this.search_input) <= parseInt(max_num) &&
            parseInt(this.search_input) > 0
          ) {
            this.searching_data = true;
    ...
    async get_more_data() {
      ...
      if (token_id_list.includes(1)) {
        ...
        for (let i = parseInt(amount); i > 0; i--) {
          ...
          var owner = await this.contract.methods.ownerOf(amount).call();
    ...
    async add_label() {
      if (this.new_label) {
        var transactionParameters = {
          ...
          from: this.wallet,
          data: this.label_contract.methods
            .addLabel(this.new_label, this.current_token_id)
            .encodeABI(), // Optional
        };
    ...
    async removeLabel() {
      var transactionParameters = {
        ...
        from: this.wallet,
        data: this.label_contract.methods
          .removeLabel(this.new_label, this.current_token_id)
          .encodeABI(), // Optional
      };
    ...
    async get_model(tokenID) {
      ...
      var result = await this.contract.methods.tokenURI(tokenID).call();
      ...
      var model_url = `https://ipfs.io/ipfs/${
    ...
    async mint2nft() {
      ...
      var transactionParameters = {
        ...
        data: this.contract.methods
          .mintNFT(this.wallet, this.metadata.url)
          .encodeABI(),
